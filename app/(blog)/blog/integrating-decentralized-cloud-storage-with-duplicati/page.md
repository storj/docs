...
metadata:
  description: "In this blog post I will share an overview of how I built the native\
    ...
    \ who do n..."
  heroimage: /blog/integrating-decentralized-cloud-storage-with-duplicati/75e382c46a1d2f4e.jpeg
...
![](/blog/integrating-decentralized-cloud-storage-with-duplicati/8cec12d26906229c.png)Then use those static strings whenever you need to return a translatable string to the user.
...
![](/blog/integrating-decentralized-cloud-storage-with-duplicati/8e24a910b91ea921.png)Additionally we are setting some custom metadata for the LastAccess and the LastModification. This metadata is later retrieved again from the file during listing and helps Duplicati verify and handle the files.
...
![](/blog/integrating-decentralized-cloud-storage-with-duplicati/bc7a64c4af5a8d43.png)For that method we needed an IFileEntry-Implementation – therefore I created a "Tardigrade file“ which fetches its properties (size, name, etc.) from the underlying Storj-object (see TardigradeFile.cs).
...
![](/blog/integrating-decentralized-cloud-storage-with-duplicati/b0bfc07431233b78.png)### Facing the user
...
![](/blog/integrating-decentralized-cloud-storage-with-duplicati/389e5f7c1c9b0d34.png)In the template-file we can now define the view shown when the user selects our back end from the list of available back ends.
...
![](/blog/integrating-decentralized-cloud-storage-with-duplicati/bec4fb180016851d.png)In line 162 you can see the call into the WebModule for Tardigrade asking for all the available Satellites. Following that we also set the default values to use if they are not yet set.

To get our options from the UI into the options that Duplicati sends to our back end, we need to map the values like this:

![](/blog/integrating-decentralized-cloud-storage-with-duplicati/4f13029531e11867.png)Now we have a clean and easy to use UI for the users of our back end, that fetches some values from the WebModule and provides all selected options to our back end.

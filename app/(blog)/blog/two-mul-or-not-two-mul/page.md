...
metadata:
 ...
 title: Two mul or not two mul
title: Two mul or not two mul
---

One of the heavy CPU uses in Storj network is Edwards 25519 signature scheme.
Downloading and uploading data needs to send signed messages from Satellite (server)
to the Storage Node (computers holding data) and need to ensure that the messages
aren't tampered with. Hence the Satellite needs to do a lot of signing and
verification; the same goes for Uplink (client application) and Storage Node.
...
between amd64 vs. arm64. The multiplication by 19.
...
[lensm](https://storj.dev/blog/lensm) and [objdump](https://pkg.go.dev/cmd/objdump)._
...
It's really helpful for indexing into arrays and sometimes to do a `x + y * k` operation.
...
At the time I was thinking, "Oh, just some optimization missing from the arm64
SSA rules". I'll test how this optimization affects things by manually replacing
the `x * 19` with `mul19`:
...
I also removed the initial caching of `a1 * 19` values as well to reduce register
pressure, which seemed to help as well. And I got these results:
...
I did a bunch of other optimizations, which overall got me to ~20% improvement
for arm64. Of course one thing did bug me, before trying to get things merged --
the compiler should do the `*19` optimization, not me.

## Trying to optimize the compiler

I thought about the optimization a bit to generalize it -- in some sense
trying to figure out optimizations that can be represented via additions
and shifts.
...
So, I created an issue report and discuss things further https://github.com/golang/go/issues/67575.

It did seem like a trivial optimization at this point, however then Erifan and
Randall pointed out that they are not really seeing the performance improvements.
Also that the cost of the shift operation can depend on the size of the shift.
...
One thing to notice about the ed25519 code is that it's quite heavy on multiplication
already. This got me thinking about, how many instructions you can do in parallel.
...
are quite clever; and similarly the CPU-s are clever and may reorder things to be better.

And the third is, how many ops we can do in parallel on a single core. Every modern
core has multiple execution units. The execution units aren't equal either;
there are two or more integer units, one or two floating point addition units,
and one or two floating point multiplication units. So, you can do an integer
addition, a floating point addition and a floating point multiplication at the same time.[<sup>Agner Fog</sup>](https://www.agner.org/optimize/optimizing_cpp.pdf)
Of course the internal details of cores can vary wildly.

I didn't measure exactly which execution units were overwhelmed and which ones
underused, but either way -- we got an performance improvement.
...
_Of course, there are additional optimizations possible. For example, there are also AVX2
and AVX512 implementations that perform better._

The improvements we'll be coming to you soon; in Go 1.25.
...
if [ "$installed_version" = "$latest_version" ] && [ "$output" != true ]; then
...
if [ "$output" = true ]; then
  cd -
  install -m 755 $tmp_dir/$unzip_dir/uplink $output_path/uplink
  echo "Downloaded and unzipped to $output_path"
  exit 0
fi
